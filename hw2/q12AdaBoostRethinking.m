...
train=load('hw2_adaboost_test.dat'); % size n*3 dimension with feature1, feature2, label
...
for m=1:size(feature1Matrix,1) %lead to high o(n), attention please
	if (m>1)
		theta1Vector(m,1)=1/2*(feature1Matrix(m,1)+feature1Matrix(m-1,1));
	else %m==1
		theta1Vector(m,1)=feature1Matrix(m,1)-1;
	end
end

for m=1:size(feature2Matrix,1)
	if (m>1)
		theta2Vector(m,1)=1/2*(feature2Matrix(m,1)+feature2Matrix(m-1,1));
	else %m==1
		theta2Vector(m,1)=feature2Matrix(m,1)-1;
	end
end
%%%s parameter setting
s(1,1)=1;
...
uNCur=(1/size(feature1Matrix,1))*ones(size(feature1Matrix,1),1);
uNnext=zeros(size(feature1Matrix,1),1);
