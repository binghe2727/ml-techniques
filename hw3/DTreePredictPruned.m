function [ gt,iMin,sMin,thetaMin,r] = DTreePredictPruned( pareNodeValue,dataMatrix ,r,order)% format of x1,x2,y matrix expansion
...
elseif (order==2)|(order==3)
    %orderNow=floor(order/2);
    gt=pareNodeValue;
    iMin=0;
    sMin=0;
    thetaMin=0;
    
    r(order,1)=pareNodeValue;
    r(order,2)=0;
    r(order,3)=0;
    r(order,4)=0;
    r(order,5)=order;
    ...
    [gtPosi1,iMinPosi1,sMinPosi1,thetaMinPosi1,r]=DTreePredict(+1,posi1PreMatr,r,2*order);% the imin,pos,just for judging?? 
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%but we should return the same
    %format for the future processing for the usage.
    [gtNega1,iMinNega1,sMinNega1,thetaMinNega1,r]=DTreePredict(-1,nega1PreMatr,r,2*order+1);
    gt=[gtPosi1;gtNega1];
